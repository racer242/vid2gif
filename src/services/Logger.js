...
class Logger {
  constructor() {
    this.params = {
      ...
      idWidth: 20, //Ширина области строки консоли под идентификатор источника данных
      sourceWidth: 50, //Ширина области строки консоли под источник данных
      ...
      undefinedErrorLog:
        "Error log isn't defined. Data will be written to system log",
      systemLogSaveError:
        "Unable to save system log on disk. Some log data may be lost",
      loopedMessageDetected: "Looped logging is temporary stopped for: ",
      consoleLevelColors: [
        //Цвета подсветки сообщений
  ...
  getMessageProps(message) {
    ...
    let matches = new RegExp(
      this.params.messageLevelMacros.source,
      this.params.messageLevelMacros.flags
    ).exec(message);
    if (matches) {
  ...
  logToConsole(ids, level, message, args) {
    if (
      ...
      let outString = [
        "\x1b[90m" +
          (date + " ...").padEnd(this.params.dateWidth, ".") +
          "\x1b[0m",
        (
          (id + "\x1b[90m" + " .").padEnd(this.params.idWidth + 7, ".") +
          "\x1b[0m" +
          " " +
          (source + "\x1b[90m" + " ...")
  ...
  async logToFile(ids, message, args) {
    if (this.logs[ids.id][ids.shadow]) {
      await this.writeToLog(
        this.logs[ids.id][ids.shadow],
        this.composeData(ids, message, args)
      );
    } else {
      await this.logError(this.params.undefinedLogError, [
        { id: id, lost: args },
      ]);
    }
  }
  ...
  trackLogContent(ids, message, args) {
    ...
    if (this.logs[ids.id][ids.shadow]) {
      ...
      let hash = Number(
        xxhash
          .h32(name + id + extId + message + args.join(" "), 0x1111)
          .toString(10)
      );
      let params;
      if (log.trackBuffer[hash]) {
        ...
        if (params.c > this.params.maxMessageRepeat) {
          if (!params.muted) {
            if (ids.shadow == 0) {
              let loopedMessageProps = this.getMessageProps(
                this.params.loopedMessageDetected +
                  this.getMessageProps(message).message
              );
              this.logToConsole(
                ids,
                loopedMessageProps.level,
                loopedMessageProps.message
              );
      ...
      params.timeout = setTimeout(
        function () {
          delete this.owner[this.hash];
        }.bind(params),
        this.params.loopedMessageTimeout
      );
      return result;
    }
  }
  ...
  startFlushTimeout(log) {
    ...
    timeout =
      this.params.flushLogTimeout +
      this.params.flushLogTimeoutVarience * Math.random();
    log.owner = this;
    ...
    if (log.cache.length > 0) {
      if (log.path) {
        if (await this.saveLog(log)) {
          ...
          await this.logError(this.params.logSaveError, [
            { id: log.id, path: log.path, lost: log.cache },
          ]);
        }
      } else {
  ...
  async saveLog(log) {
    ...
    if (
      this.params.logArchiveLocation &&
      this.params.logArchiveLocation != "" &&
      getFileSize(path) > this.params.maxFileSize
    ) {
      archiveName = this.params.logArchiveFileName
    ...
    if (archiveName) {
      data =
        this.params.archivedLogMessage.replace("{archiveName}", archiveName) +
        "\n" +
        data;
    }
    let logStream;
  ...
  init(params) {
    ...
    if (this.params.useConsole) {
      console.log(
        this.params.startMessage.replace(
          "$timestamp$",
          new Date().toLocaleString()
        )
      );
